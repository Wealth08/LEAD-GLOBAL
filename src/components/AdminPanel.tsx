...
import { UserProfile, Transaction, TaskLog, SupportTicket } from '../types';
...
export default function AdminPanel({ onNavigate }: AdminPanelProps) {
  ...
  const [selectedUserVerified, setSelectedUserVerified] = useState(true);
  const [selectedUserStatus, setSelectedUserStatus] = useState<
    'Active' | 'Pending Verification' | 'Banned'
  >('Active');

  // Rejection reason inputs
  const [rejectReason, setRejectReason] = useState(
    'Screenshot is blurry or hash code does not match the wire transfer reference.'
  );
  const [kycRejectReason, setKycRejectReason] = useState(
    'Document text is cropped or selfie photo does not match the ID submitted.'
  );
  ...
  const [adminTab, setAdminTab] = useState<
    'users' | 'deposits' | 'withdrawals' | 'kyc_docs' | 'tasks_verify' | 'broadcast' | 'transactions'
  >('users');
  ...
  const selectUserForEdit = (u: UserProfile) => {
    ...
    setSelectedUserBalance(u.walletBalance.toString());
    setSelectedUserVerified(u.isEmailVerified);
    setSelectedUserStatus(u.status);
  };
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    await withLoading('edit_user', async () => {
      const res = await dbService.adminAdjustUserFields(
        selectedUserId,
        parseFloat(selectedUserBalance) || 0,
  ...
  const handleTriggerBroadcastSubmit = async (e: React.FormEvent) => {
    ...
    await withLoading('broadcast', async () => {
      await dbService.adminPostAnnouncement(announcementText);
      showToastMsg('Announcement broadcast dispatched to all channels!');
      setAnnouncementText('');
    });
  };
          ...
          <div className="bg-[#111112] p-4 rounded-2xl border border-white/[0.04]">
            ...
            <nav className="flex flex-col gap-1.5 font-mono text-xs">
              {[
                ...
                { id: 'deposits', label: 'Deposit Receipt Verification', badge: pendingDepositsList.length > 0 ? pendingDepositsList.length.toString() : null, badgeColor: 'bg-red-900 text-white rounded-full w-4 h-4 flex items-center justify-center text-[9px] animate-pulse' },
            ...
            <div className="bg-[#111112] p-6 rounded-3xl border border-white/[0.04] space-y-6">
              ...
              {selectedUserId && (
                <form
                  ...
                  className="p-4 bg-[#1C170E] rounded-2xl border border-[#D4AF37]/30 space-y-4 font-mono text-xs"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-[#D4AF37] uppercase">
                      Editing: {users.find((u) => u.id === selectedUserId)?.email}
                    </span>
                    <button
                      type="button"
                  ...
                  <div className="grid sm:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        type="number"
                        value={selectedUserBalance}
                        onChange={(e) => setSelectedUserBalance(e.target.value)}
                        className="w-full bg-[#0C0C0D] border border-[#D4AF37]/25 text-[#D4AF37] rounded-lg py-1.5 px-3 focus:outline-none"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-[#0C0C0D] border border-white/10 rounded-lg py-1.5 px-2 focus:outline-none text-white"
                      >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2.5">
                        <input
                          type="checkbox"
                          checked={selectedUserVerified}
                          onChange={(e) => setSelectedUserVerified(e.target.checked)}
                          className="rounded text-[#D4AF37] focus:ring-0 w-4 h-4"
                        />
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#D4AF37] text-black font-bold uppercase rounded text-[10px] tracking-wider flex items-center gap-2 disabled:opacity-60"
                  >
              ...
              {pendingTasksList.length === 0 ? (
                ...
                <div className="space-y-4">
                  {pendingTasksList.map((row) => (
                    <div
                      key={row.id}
                      className="p-4 bg-[#0C0C0D] border border-white/[0.03] rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                    >
                      <div className="flex-1">
                        <span className="text-[10px] text-[#D4AF37] font-mono uppercase font-bold block">
                          Task: {row.taskTitle}
                        </span>
                        <p className="text-xs font-bold text-white mt-1">
                          Submitted by:{' '}
                          {users.find((u) => u.id === row.userId)?.email || row.userId}
                        </p>
                        <code className="text-[10px] text-gray-500 font-mono mt-1 block break-all">
                          Proof: {row.proof}
                        </code>
                        <span className="text-[9px] text-gray-600 font-mono block mt-1">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button
                          onClick={() => handleApproveTaskLog(row.id)}
                          disabled={actionLoading === `approve_task_${row.id}`}
                          className="px-3 py-1.5 bg-emerald-500 text-black font-semibold text-[9.5px] uppercase font-mono rounded hover:brightness-110 transition flex items-center gap-1.5 disabled:opacity-60"
                        >
                          {actionLoading === `approve_task_${row.id}` ? (
                            <RefreshCw className="w-3 h-3 animate-spin" />
                          ) : (
                            'Credit Reward'
                          )}
                        ...
                        <button
                          onClick={() => handleRejectTaskLog(row.id)}
                          disabled={actionLoading === `reject_task_${row.id}`}
                          className="px-3 py-1.5 bg-red-950 text-red-400 border border-red-500/10 text-[9.5px] uppercase font-mono rounded hover:bg-red-900 transition flex items-center gap-1.5 disabled:opacity-60"
                        >
                          {actionLoading === `reject_task_${row.id}` ? (
                            <RefreshCw className="w-3 h-3 animate-spin" />
                          ) : (
                            'Deny'
                          )}
          ...
          {adminTab === 'broadcast' && (
            <div className="bg-[#111112] p-6 rounded-3xl border border-white/[0.04] space-y-6">
              <div className="pb-3 border-b border-white/[0.03]">
                ...
                <p className="text-xs text-gray-400">
                  Publish global announcements visible to all platform users.
                </p>
              </div>
              ...
              <form onSubmit={handleTriggerBroadcastSubmit} className="space-y-4">
                <div>
                  ...
                  <textarea
                    required
                    rows={4}
                    className="w-full bg-[#1A1A1C] border border-white/[0.06] rounded-xl p-3 text-xs text-white focus:outline-none focus:border-[#D4AF37] transition"
                    value={announcementText}
                    onChange={(e) => setAnnouncementText(e.target.value)}
                    placeholder="Enter announcement text to display on LeadsGlobal screens…"
                  />
                </div>
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-gradient-to-r from-[#D4AF37] to-[#8C6D23] text-black font-bold font-mono text-xs uppercase tracking-wider rounded-xl transition hover:brightness-110 disabled:opacity-60 flex items-center gap-2"
                >
              ...
              <div className="bg-[#0C0C0D] p-5 rounded-2xl border border-white/[0.03] space-y-3 font-mono text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-[#D4AF37] uppercase">
                    Recent Time-Simulation Activity:
                  </span>
                </div>
                ...
                <div className="max-h-48 overflow-y-auto space-y-1.5 pr-1">
                  {simulationLogs.length === 0 ? (
                    <span className="text-gray-500 italic text-[11px]">
                      No simulations run yet. Use the sidebar Fast-Forward widget.
                    </span>
                  ) : (
                    simulationLogs.map((logStr, idx) => (
                      <p
                        key={idx}
                        className="text-[10px] text-gray-400 border-l border-[#D4AF37]/30 pl-2 py-0.5"
                      >
                        {logStr}
                      </p>
                    ))
